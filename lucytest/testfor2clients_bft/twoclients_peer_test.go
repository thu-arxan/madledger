...
func TestBFTCreateTx2PB(t *testing.T) {
	for m := 0; m < 8; m++ {
		...
		tx, err := types.NewTx(channel, common.ZeroAddress, contractCodes, 0, "", bftClients[0].GetPrivKey())
		require.NoError(t, err)
		...
		tx, err = types.NewTx(channel, common.ZeroAddress, contractCodes, 0, "", bftClients[1].GetPrivKey())
		require.NoError(t, err)
