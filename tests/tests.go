...
func testCreateChannel(t *testing.T, client *client.Client, peers []*types.Member) {
...
func callContract(t *testing.T, channelID string, client *client.Client) {
	...
	tx, _ := types.NewTx(channelID, contractAddress, payload, 0, "", client.GetPrivKey())
	status, err := client.AddTx(tx)
	require.NoError(t, err)
	...
	tx, _ = types.NewTx(channelID, invalidAddress, []byte("invalid"), 0, "", client.GetPrivKey())
	status, err = client.AddTx(tx)
	require.NoError(t, err)
