...
func TestInsert(t *testing.T) {
	// 1 -> 1,1
	var waits = insert(newWaitList(1), &waitChan{num: 1})
	require.Len(t, waits, 2)
	require.EqualValues(t, waits[0].num, 1)
	require.EqualValues(t, waits[1].num, 1)
	require.NotEqual(t, fmt.Sprint(&waits[0]), fmt.Sprint(&waits[1]))
	// 1 -> 0,1
	waits = insert(newWaitList(1), &waitChan{num: 0})
	require.Len(t, waits, 2)
	require.EqualValues(t, waits[0].num, 0)
	require.EqualValues(t, waits[1].num, 1)
	// 1 -> 1,2
	waits = insert(newWaitList(1), &waitChan{num: 2})
	require.Len(t, waits, 2)
	require.EqualValues(t, waits[0].num, 1)
	require.EqualValues(t, waits[1].num, 2)
	// 1,1 -> 1,1,2
	waits = insert(newWaitList(1, 1), &waitChan{num: 2})
	...
	// 1,1 -> 1,1,1
	waits = insert(newWaitList(1, 1), &waitChan{num: 1})
	require.Len(t, waits, 3)
	require.EqualValues(t, waits[0].num, 1)
	require.EqualValues(t, waits[1].num, 1)
	require.EqualValues(t, waits[2].num, 1)
	require.NotEqual(t, fmt.Sprint(&waits[0]), fmt.Sprint(&waits[1]))
	require.NotEqual(t, fmt.Sprint(&waits[0]), fmt.Sprint(&waits[2]))
	// 1,1 -> 0,1,1
	waits = insert(newWaitList(1, 1), &waitChan{num: 0})
	require.Len(t, waits, 3)
	require.EqualValues(t, waits[0].num, 0)
	require.EqualValues(t, waits[1].num, 1)
	require.EqualValues(t, waits[2].num, 1)
	// 1,3 -> 1,2,3
	waits = insert(newWaitList(1, 3), &waitChan{num: 2})
	require.Len(t, waits, 3)
	require.EqualValues(t, waits[0].num, 1)
	require.EqualValues(t, waits[1].num, 2)
	require.EqualValues(t, waits[2].num, 3)
	// 1,3,3,5 -> 1,3,3,4,5
	waits = insert(newWaitList(1, 3, 3, 5), &waitChan{num: 4})
	...
	// 1,3,3,5 -> 1,3,3,5,7
	waits = insert(newWaitList(1, 3, 3, 5), &waitChan{num: 7})
	require.Len(t, waits, 5)
	require.EqualValues(t, waits[0].num, 1)
	require.EqualValues(t, waits[1].num, 3)
	require.EqualValues(t, waits[2].num, 3)
